...
IRAsmStmt::IRAsmStmt(const ASTContext &C, SourceLocation asmloc,
                     ...
                     StringLiteral *asmstr, unsigned numclobbers,
                     StringLiteral **clobbers, SourceLocation rparenloc)
  : AsmStmt(IRAsmStmtClass, asmloc, issimple, isvolatile, numoutputs,
            numinputs, numclobbers), RParenLoc(rparenloc), AsmStr(asmstr) {
...
char IRAsmStmt::AsmStringPiece::getModifier() const {
  assert(isOperand() && "Only Operands can have modifiers.");
  return isLetter(Str[0]) ? Str[0] : '\0';
}

StringRef IRAsmStmt::getClobber(unsigned i) const {
  return getClobberStringLiteral(i)->getString();
}
...
/// translate this into a numeric value needed to reference the same operand.
/// This returns -1 if the operand name is invalid.
int IRAsmStmt::getNamedOperand(StringRef SymbolicName) const {
  unsigned NumPlusOperands = 0;
...
/// Assemble final IR asm string (IR-style).
