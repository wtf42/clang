...
void CodeGenFunction::EmitIRAsmStmt(const IRAsmStmt &S) {
  ...
  std::unique_ptr<llvm::Module> emptyFuncMod =
    llvm::make_unique<llvm::Module>("ir_inline_asm", getLLVMContext());
  ...
  llvm::BasicBlock *BB =
    llvm::BasicBlock::Create(emptyFuncMod->getContext(), entryStr, emptyFunc);
  ...
  std::unique_ptr<llvm::Module> parsedFuncMod =
    llvm::parseAssemblyString(modStr, Error, getLLVMContext());
  if (!parsedFuncMod) {
    std::string ErrMsg;
  ...
  for (llvm::inst_iterator I = inst_begin(parsedFunc),
       E = inst_end(parsedFunc); I != E; ++I) {
    if (llvm::ReturnInst* ret = dyn_cast<llvm::ReturnInst>(&*I)) {
      if (hasReturnInst) {
        llvm::report_fatal_error(
          "ir inline asm should have only one return instruction");
      }
      if (ret->getNumOperands() != 1) {
        llvm::report_fatal_error(
          "ir inline asm should have one return operand");
      }
      ...
      if (ResultRegTypes.size() == 1) {
        ...
        for (unsigned i = 0, e = ResultRegTypes.size(); i != e; ++i) {
          llvm::Value *Tmp =
            Builder.CreateExtractValue(Result, i, "asmresult");
      ...
      for (unsigned i = 0, e = RegResults.size(); i != e; ++i) {
        ...
        // If the result type of the LLVM IR asm doesn't match the result type
        // of the expression, do the conversion.
        if (ResultRegTypes[i] != ResultTruncRegTypes[i]) {
          llvm::Type *TruncTy = ResultTruncRegTypes[i];

          // Truncate the integer result to the right size, note that TruncTy
          // can be a pointer.
    ...
    std::string sourceLocation = S.getSourceRange().getBegin()
      .printToString(getContext().getSourceManager());
    i->setMetadata("ir_asm",
      llvm::MDNode::get(getLLVMContext(),
      llvm::MDString::get(getLLVMContext(), "at " + sourceLocation)));
    Builder.Insert(i);
  }
}
